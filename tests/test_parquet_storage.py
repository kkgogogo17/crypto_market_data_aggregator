...
class TestParquetStorage:
    ...
    def test_init_with_default_data_dir(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            storage = ParquetStorage()
            assert (
                str(storage.local_data_dir) == "data"
            )  # Path constructor converts './data' to 'data'
    ...
    def test_init_sets_r2_config(self):
        ...
        with patch.dict("os.environ", env_vars):
            ...
            assert (
                storage.r2_config["aws_access_key_id"] == env_vars["R2_ACCESS_KEY_ID"]
            )
            assert (
                storage.r2_config["aws_secret_access_key"]
    ...
    def test_read_from_parquet_success(self, temp_data_dir):
        ...
        with patch.dict("os.environ", {"LOCAL_DATA_DIR": str(temp_data_dir)}):
            ...
            data = [
                {"date": "2024-01-01T00:00:00.000Z", "open": 45000.5, "close": 45050.0}
            ]
            _ = storage.save_to_parquet(data, "BTCUSD", "2024-01-01")
...
class TestParquetStorageFunctions:
    ...
    def test_save_crypto_data_to_parquet_success(
        self, sample_api_response, temp_data_dir
    ):
        """Test successful saving of API response to parquet"""
        with patch.dict("os.environ", {"LOCAL_DATA_DIR": str(temp_data_dir)}):
            result = save_crypto_data_to_parquet(
    ...
    def test_save_crypto_data_to_parquet_empty_price_data(
        self, empty_api_response, temp_data_dir
    ):
        """Test handling of API response with empty price data"""
        with patch.dict("os.environ", {"LOCAL_DATA_DIR": str(temp_data_dir)}):
            result = save_crypto_data_to_parquet(
                empty_api_response, "BTCUSD", "2024-01-01"
            )
    ...
    def test_save_crypto_data_to_parquet_invalid_format(self, temp_data_dir):
        ...
        with patch.dict("os.environ", {"LOCAL_DATA_DIR": str(temp_data_dir)}):
            ...
            result = save_crypto_data_to_parquet(
                invalid_response, "BTCUSD", "2024-01-01"
            )

            assert "error" in result
            assert "Invalid API response format" in result["error"]

    def test_read_crypto_data_from_parquet_success(
        self, sample_api_response, temp_data_dir
    ):
    ...
    def test_upload_parquet_to_r2_with_auto_key(self, mock_upload):
        ...
        mock_upload.assert_called_once_with(
            test_path, "crypto-data/2024/01/01/BTCUSD_20240101.parquet"
        )
        assert result["success"] is True
    ...
    def test_upload_parquet_to_r2_with_custom_key(self, mock_upload):
        ...
        _ = upload_parquet_to_r2(test_path, custom_key)
